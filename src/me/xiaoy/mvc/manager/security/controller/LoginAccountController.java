...
public class LoginAccountController extends BaseController {
	...
	/**
	 * 用户登录， 记录登录用户名、密码、ip， 反复登录失败视为危险行为， 进行阻止
	 * 
	 * @param request
	 * @param merchant
	 * @return
	...
	public String index(LoginAccount account) {
		...
		request.setAttribute(SessionKey.LOGIN_ACCOUNT, account);
		...
		try {
			...
			saveSessionMsg("登录认证失败");
			logService.save(account, ip, "登录认证失败");
			return "redirect:login";
		}
		if (currentUser.isAuthenticated()) {
			session.setAttribute(SessionKey.LOGIN_ACCOUNT, account);
			request.setAttribute("welcome", DateTimeUtil.getWelcome());
			logService.save(account, ip, "用户登录成功");
		} else {
			saveSessionMsg("登录权限认证失败");
			logService.save(account, ip, "登录权限认证失败");
			return "redirect:login";
		}
